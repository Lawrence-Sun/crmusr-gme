...
// 新版本：PDR 分辨率是方位角的分辨率
auto add_resolution_to_alpha_azimuthal(TRandom &random_generator, double alpha_michel_muon, double LGA_resolution, double PDR_azimuthal_resolution)
{
    double MDD1_h{50.0};  // 第一层距离靶中心的高度
    double MDD2_h{400.0}; // 第二层距离靶中心的高度
    double PDR_R{150.0};  // PDR的半径
    ...
    // 然后我们要生成Michel电子击中PDR的坐标
    double gamma = random_generator.Uniform(0, 2 * M_PI); // 定义圆锥曲面相对方位角
    // 加入方位角分辨率：对 gamma 加高斯噪声
    gamma += random_generator.Gaus(0, PDR_azimuthal_resolution);
    ...
    // PDR 不再加位置分辨率，因为现在是方位角分辨率
    // PDR_hit_x += random_generator.Gaus(0, PDR_resolution); // 注释掉
    // PDR_hit_y += random_generator.Gaus(0, PDR_resolution);
    // PDR_hit_z += random_generator.Gaus(0, PDR_resolution);
...
// 新版本：使用方位角分辨率的 get_single_resolution_michel
auto get_single_resolution_michel_azimuthal(double LGA_resolution, double PDR_azimuthal_resolution)
{
    TFile eigen_michel_dis("/Users/sunmingchen/codeDIR/GME/dev/megaMichelDis.root", "READ");
    auto pi_N_Michel = (TH1D *)eigen_michel_dis.Get("piMichelDis");
    auto pi_P_Michel = (TH1D *)eigen_michel_dis.Get("piMichelDisAnti");
    ...
    for (int i = 0; i < random_number; i++)
    {
        ...
        double pi_N_reconstruct_cos_alpha = add_resolution_to_alpha_azimuthal(random_generate, pi_N_cos_alpha, LGA_resolution, PDR_azimuthal_resolution);
        double pi_P_reconstruct_cos_alpha = add_resolution_to_alpha_azimuthal(random_generate, pi_P_cos_alpha, LGA_resolution, PDR_azimuthal_resolution);
        double ka_N_reconstruct_cos_alpha = add_resolution_to_alpha_azimuthal(random_generate, ka_N_cos_alpha, LGA_resolution, PDR_azimuthal_resolution);
        double ka_P_reconstruct_cos_alpha = add_resolution_to_alpha_azimuthal(random_generate, ka_P_cos_alpha, LGA_resolution, PDR_azimuthal_resolution);
    ...
    TFile output_file("single_resolution_michel_azimuthal.root", "RECREATE");
    pi_N_Michel_resolution->Write();
    pi_P_Michel_resolution->Write();
    ka_N_Michel_resolution->Write();
    ka_P_Michel_resolution->Write();
...
// 新版本：使用方位角分辨率的 generate_single_resolution_chi2
auto generate_single_resolution_chi2_azimuthal(double LGA_resolution, double PDR_azimuthal_resolution)
{
    get_single_resolution_michel_azimuthal(LGA_resolution, PDR_azimuthal_resolution);
    // 指定事例数
    int number_of_events{300000};

    TFile eigen_michel_dis("single_resolution_michel_azimuthal.root", "READ");
    auto pi_P_Michel_resolution = (TH1D *)eigen_michel_dis.Get("pi_P_Michel_resolution");
    auto ka_P_Michel_resolution = (TH1D *)eigen_michel_dis.Get("ka_P_Michel_resolution");
    ...
    eigen_michel_dis.Close();
    double one_sigma_width = max_ratio - min_ratio;
    // cout << "LGA resolution: " << LGA_resolution << ", PDR azimuthal resolution: " << PDR_azimuthal_resolution
    //      << ", One sigma width: " << one_sigma_width << endl;
    return one_sigma_width;
}
...
int main()
{
    ...
    // auto LGA_step = (30 - 3) / grid_size;  // 从3mm到30mm，分成50个点
    // auto PDR_step = (50 - 10) / grid_size; // 从10mm到50mm，分成50个点
    // // 生成放置结果的二维直方图
    // TH2D *resolution = new TH2D("resolution", "resolution", grid_size, 3, 30, grid_size, 10, 50);
    // for (int i = 0; i < grid_size; i++)
    // {
    //     double LGA_resolution = 3 + i * LGA_step; // 从3mm到30mm
    //     for (int j = 0; j < grid_size; j++)
    //     {
    //         double PDR_resolution = 10 + j * PDR_step; // 从10mm到50mm
    ...
    // int grid_size_azimuthal{10};
    // auto LGA_resolution{4.0};
    // auto PDR_step_azimuthal = (15 - 1) / grid_size_azimuthal * M_PI / 180.0;
    // TH1D *resolution_azimuthal = new TH1D("resolution_azimuthal", "resolution_azimuthal", grid_size_azimuthal, 1 * M_PI / 180.0, 15 * M_PI / 180.0);
    // for (int i = 0; i < grid_size_azimuthal; i++)
    // {
    //     double PDR_azimuthal_resolution = 1 * M_PI / 180.0 + i * PDR_step_azimuthal;
    //     auto one_sigma_width_azimuthal = generate_single_resolution_chi2_azimuthal(LGA_resolution, PDR_azimuthal_resolution);
    //     resolution_azimuthal->SetBinContent(i + 1, one_sigma_width_azimuthal);

    //     // 打印进度条
    //     int total = grid_size_azimuthal;
    //     int current = i + 1;
    //     int barWidth = 10; // 进度条宽度
    //     double progress = (double)current / total;
    //     std::cout << "\r[";
    //     int pos = barWidth * progress;
    //     for (int k = 0; k < barWidth; ++k)
    ...
    // // resolution->Write();
    // resolution_azimuthal->Write();
    // output_file.Close();

    generate_single_resolution_chi2(10, 30);
    // generate_single_resolution_chi2_azimuthal(10, 10 * M_PI / 180.0);
